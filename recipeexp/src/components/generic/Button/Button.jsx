...
Button.propTypes = {
  variation: PropTypes.oneOf(['primary', 'success', 'secondary', 'danger', 'warning']),
...
function Button({ children, isLoading, type, variation, rounded, outline, className, ...rest }) {
  const isPrimary = variation === 'primary';
  const isSecondary = variation === 'secondary';
  const isSuccess = variation === 'success';
  ...
  const classes = classNames(className, 'px-3 py-1.5 border flex items-center  justify-center', {
    ...
    'text-green-500': outline && isSuccess,
    'text-yellow-400': outline && isWarning,
    'text-red-500': outline && isDanger,
  });
  return (
    <button type={type} {...rest} disabled={isLoading} className={classes}>
      {isLoading && (
        <div role="status">
          <svg
            aria-hidden="true"
            className="w-[1rem] h-[1rem] mr-2 animate-spin fill-slate-400"
            viewBox="0 0 100 101"
            fill="none"
